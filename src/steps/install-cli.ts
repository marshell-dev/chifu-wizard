...
// the agent skill can still fall back to `bunx @mfinikov/chifu` at runtime.
...
export async function installCli(prompt: Prompter, assumeYes: boolean): Promise<CliResult> {
  ...
  if (!proceed) {
    log.skip("Skipped CLI install — your agent can still use `bunx @mfinikov/chifu`");
    return { present: false, installedVia: null };
  }

  // Try npm first (broadest reach), then bun.
  if (onPath("npm")) {
    log.info("Installing via npm (npm i -g @mfinikov/chifu)…");
    const r = run("npm", ["install", "-g", "@mfinikov/chifu"], { capture: true });
    if (r.ok && chifuWorks()) {
  ...
  if (onPath("bun")) {
    log.info("Installing via bun (bun add -g @mfinikov/chifu)…");
    const r = run("bun", ["add", "-g", "@mfinikov/chifu"], { capture: true });
    if (r.ok && chifuWorks()) {
  ...
  log.fail(
    "Couldn't install the chifu CLI automatically. Install it yourself with " +
      `${c.bold("npm i -g @mfinikov/chifu")} or ${c.bold("bun add -g @mfinikov/chifu")}.`,
  );
  log.info("Your agent can still run `bunx @mfinikov/chifu check` without a global install.");
  return { present: false, installedVia: null };
}
