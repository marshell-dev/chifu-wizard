...
1. **Installs the chifu CLI.** Detects whether `chifu` is on your PATH; if not,
   installs it globally with `npm i -g chifu` (falling back to `bun add -g
   chifu`) and tells you which one it used. The CLI also works via `bunx chifu`
